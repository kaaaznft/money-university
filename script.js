...
// Target audience system (kids vs adults)
let currentTarget = 'adult'; // 'adult' or 'kids'
...
const termTranslations = {
    ...
    '複利': {
        adult: '複利',
        kids: 'ふくり（おかねがどんどんふえる仕組み）'
    },
    '分散投資': {
        adult: '分散投資',
        kids: 'ぶんさんとうし（いろんなところにとうしすること）'
    },
    '為替レート': {
        adult: '為替レート',
        kids: 'かわせレート（外国のおかねとの交換）'
    },
    '消費税': {
        adult: '消費税',
        kids: 'しょうひぜい（買い物のときのぜいきん）'
    },
    '所得税': {
        adult: '所得税',
        kids: 'しょとくぜい（働いてもらったおかねのぜいきん）'
    },
    '住民税': {
        adult: '住民税',
        kids: 'じゅうみんぜい（住んでいる町に払うぜいきん）'
    },
    '仮想通貨': {
        adult: '仮想通貨',
        kids: 'かそうつうか（デジタルのおかね）'
    },
    ...
    '初心者にもわかりやすい解説と楽しい計算機能で、お金の基本知識を身につけましょう。': {
        adult: '初心者にもわかりやすい解説と楽しい計算機能で、お金の基本知識を身につけましょう。',
        kids: 'はじめての人にもわかりやすい説明と楽しい計算で、おかねの基本を覚えよう！'
    },
    '通貨と税金の仕組みを<br>楽しく学ぼう！': {
        adult: '通貨と税金の仕組みを<br>楽しく学ぼう！',
        kids: 'おかねとぜいきんの仕組みを<br>楽しく学ぼう！'
    },
    '日本では国税と地方税を合わせて約50種類の税金があります。私たちの生活に関わる主要な税金について詳しく学びましょう。': {
        adult: '日本では国税と地方税を合わせて約50種類の税金があります。私たちの生活に関わる主要な税金について詳しく学びましょう。',
        kids: '日本にはたくさんの種類のぜいきんがあります。みんなの生活に関係のある大切なぜいきんについて学ぼう！'
    }
};
...
function switchTarget(target) {
    ...
    const toggleBtn = document.getElementById('target-toggle');
    const toggleText = document.getElementById('target-text');
    const toggleIcon = toggleBtn.querySelector('.toggle-icon');
    
    if (target === 'kids') {
        toggleBtn.classList.add('kids-mode');
        toggleText.textContent = '子ども向け';
        toggleIcon.textContent = '👶';
    } else {
        toggleBtn.classList.remove('kids-mode');
        toggleText.textContent = '大人向け';
        toggleIcon.textContent = '👥';
    }
    
    // Save preference
    localStorage.setItem('targetAudience', target);
...
function updatePageText() {
    // Update all text nodes recursively
    updateTextNodes(document.body);
    
    // Update specific elements that might have innerHTML
...
function updateTextNodes(element) {
    if (element.nodeType === Node.TEXT_NODE) {
        ...
        if (text && termTranslations[text]) {
            element.textContent = termTranslations[text][currentTarget];
        }
    } else {
        // For elements, check their direct text content
        const children = Array.from(element.childNodes);
        children.forEach(child => {
            if (child.nodeType === Node.TEXT_NODE) {
                const text = child.textContent.trim();
                if (text && termTranslations[text]) {
                    child.textContent = termTranslations[text][currentTarget];
                }
...
function updateSpecificElements() {
    // Update elements with innerHTML that contain translations
    const heroTitle = document.querySelector('.hero-title');
    if (heroTitle) {
        const titleText = heroTitle.innerHTML;
        if (termTranslations[titleText]) {
            heroTitle.innerHTML = termTranslations[titleText][currentTarget];
        }
    }
    ...
    if (heroDescription) {
        ...
        if (termTranslations[descText]) {
            heroDescription.textContent = termTranslations[descText][currentTarget];
        }
    }
    ...
    sectionDescriptions.forEach(desc => {
        ...
        if (termTranslations[text]) {
            desc.textContent = termTranslations[text][currentTarget];
        }
    });
    ...
    if (navBrand) {
        ...
        if (termTranslations[brandText]) {
            navBrand.textContent = termTranslations[brandText][currentTarget];
        }
    }
}

function initTargetToggle() {
    // Load saved preference
    const savedTarget = localStorage.getItem('targetAudience') || 'adult';
    switchTarget(savedTarget);
    
    // Add click handler
    const toggleBtn = document.getElementById('target-toggle');
    if (toggleBtn) {
        toggleBtn.addEventListener('click', () => {
            const newTarget = currentTarget === 'adult' ? 'kids' : 'adult';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize target audience toggle
    initTargetToggle();
});
